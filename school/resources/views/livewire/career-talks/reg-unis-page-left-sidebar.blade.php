        ...
        <div class="card-body align-middle">
            ...
            <div class="d-flex w-100 justify-content-between flex-wrap">
                @foreach ($sessions as $count => $session)
                    <div
                        class="w-20 m-1 py-1 {{!empty($session->university_id)|| !empty($session->agent_id) ? 'bg-green ':"bg-light-gray primary-text" }}  text-center ">
                        {{$loop->iteration}}
                    </div>
                @endforeach
            </div>
    ...
    <div class="card mt-4">
        <div class="card-body align-middle">
            ...
            @forelse ($reserevedSessions as $count => $session)
                @php
                    $university = $session->university ?? $session->agent;
                @endphp
                <div class="row px-2 my-2">
                    <div class="col-4 p-0" style="height: 100%; align-self: center">
                        <img src="{{$university?->logo_url}}" alt="logo" style="width: 100%;" />
                    </div>
                    <div class="col-8 pe-0">
                        <p class="small primary-text mb-0">{{$university->university_name ?? $university->name }}</p>

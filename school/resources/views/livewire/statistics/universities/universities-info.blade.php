<div x-data="{}">
    <div class="row">
        <div class="col-12">
            <div class="h4 text-blue">@lang('Students Selected ')
                {{$selected_university?->university_name ?: __('Universities')}}
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-12">
            <form method="post" wire:submit.prevent="save" onkeydown="return event.key != 'Enter';">
                ...
                <div class="row">
                    <div class="col-lg-6 mb-3">
                        <select wire:model="university_id" name="university_id"
                            ...
                            @foreach($universities as $university)
                                <option
                                    value="{{$university->id}}">{{$university?->translated_name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tr>
                                ...
                                <td class="w-50">@lang('Universities')</td>
                            </tr>
                            <!--End Heading row-->
                            @forelse($students as $student)
                                @php
                                    $university = $student->preferredUniversities->first();
                                    $universities_count = $student->preferredUniversities->count() - 1;
                                    $studentUniversity = $selected_university ?: $university;
                                @endphp
                                <tr>
                                    <td>{{$loop->index + 1 + ($students->currentPage()-1) * $students->perPage()}}</td>
                                    <td>{{$student->id}}</td>
                                    <td><img src="{{$student->profile_photo_url}}" class="navbar-user-avatar"></td>
                                    <td>
                                        <a href="javascript:void(0)" class="blue"
                                           @click="$wire.emit('showStudentDetails','{{$student->id}}')">{{$student->name}}</a>
                                    </td>
                                    <td>
                                        {{ $studentUniversity->university_name }}
            ...
            <div class="col-12 my-2 small5">
                <label
                    class="primary-text">{{$students->total()}} @lang('students has select') {{!empty($selected_university?->translated_name) ? $selected_university?->translated_name : $selected_university?->university_name}}</label>
            </div>

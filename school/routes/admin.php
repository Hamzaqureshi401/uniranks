...
Route::middleware(['setup-locale','add-session-history','firewall.all', 'auth:sanctum', config('jetstream.auth_session')])
        ...
        Route::middleware('school-selected')->group(function () {
            ...
            Route::name('school.')->prefix('school')->group(function () {
                ...
                Route::name('fair.')->prefix('fair')->controller(FairsController::class)->group(function () {
                    ...
                    Route::get('create', 'create')->name('create')->middleware('school-profile-completed');
                ...
                Route::name('students.')->prefix('students')->group(function () {
                    Route::get('/', function (){
                        return Redirect::route('admin.school.students.list');
                    })->name('show');
                    Route::view('list', 'pages.students.my-students')->name('list');
                    Route::get('print-qr', function (\Illuminate\Http\Request $request){
                        $student_id = $request->input('student_id');
                        $student = \App\Models\User::whereId($student_id)->with(['userBio.studyLevel','userBio.country','userBio.city','userBio.studyStatus'])->first();
                        return view('pages.students.print-qr',compact('student'));
                    })->name('printQr');
                    Route::get('print-all-qr', function (\Illuminate\Http\Request $request){
                        $search_destination = $request->input('search_destination');
                        $search_major = $request->input('search_major');
                        $search_university = $request->input('search_university');
                        $search_budget = $request->input('search_budget');
                       $students =   \App\Models\User::whereSelectedSchoolStudents()
                           ->with(['userBio.studyLevel','userBio.country','userBio.city','userBio.studyStatus'])
                            ->when(!empty($search_destination), fn($query) => $query->whereRelation('studyDestinations', 'country_id', $search_destination))
                            ->when(!empty($search_major), fn($query) => $query->whereRelation('majors', 'major_id', $search_major))
                            ->when(!empty($search_university), fn($query) => $query->whereRelation('preferredUniversities', 'university_id', $search_university))
                            ->when(!empty($search_budget), fn($query) => $query->whereRelation('userBio', 'fee_range_id', $search_budget))
                            ->orderBy('id')->get();
                        return view('pages.students.print-qr-for-all',compact('students'));
                    })->name('printQrAll');

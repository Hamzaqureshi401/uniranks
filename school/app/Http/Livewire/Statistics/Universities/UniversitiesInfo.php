...
class UniversitiesInfo extends Component
{
    ...
    public function loadStudents($reset_pagination=true) : LengthAwarePaginator
    {
        ...
        return User::whereSelectedSchoolStudents()->with(['preferredUniversities'])
        ->when(!empty($this->university_id),
            fn($query)=>$query->whereRelation('preferredUniversities','university_id',$this->university_id),
            fn($query)=>$query->whereHas('preferredUniversities')
        )->paginate(10);
    }
}

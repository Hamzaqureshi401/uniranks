...
use App\Models\PointingSystem\PointSource;
...
class  SmCreditHistory extends Component
{
    ...
    public function mount()
    {
        ...
        $this->balance_total = floatval($this->credit_total) - floatval($this->points_out_total);
        $this->select_source = PointSource::all();
    }
    ...
    public function render()
    {
        ...
        $histories = $this->loadHistory(false);
        $this->select_transaction = PointType::when(!empty($this->search_source), fn($query)=>$query->whereRelation('source', 'source_id', $this->search_source), fn($query)=>$query->whereHas('source'))
            ->get();
    ...
    public function loadHistory($reset_pagination=true) : LengthAwarePaginator
    {
        if ($reset_pagination){
            $this->resetPage();
        }
        ...
        */
        ...
        $query = \Auth::user()->selected_school->pointsHistory()->with(['pointType'])
            ->when(!empty($this->search_source), fn($query)=>$query->whereRelation('pointType', 'source_id', $this->search_source))
            ->when(!empty($this->search_transaction), fn($query)=>$query->where('point_type_id',$this->search_transaction))

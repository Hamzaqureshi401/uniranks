...
if (!function_exists('languages')) {
    ...
    function languages(): mixed
    {
        return Cache::rememberForever('languages-cache', function () {
            return Language::whereAvailable(1)->get();
        });
    }
}
...
if(!function_exists('calculateSchoolTotalSM')){
    ...
    function calculateSchoolTotalSM(School $school = null): float|string
    {
        ...
        $sums = $school->pointsHistory()->selectRaw('SUM(points_in) as total_points_in,SUM(points_out) as total_points_out')->first(['total_points_in','total_points_out']);
        $points_in_total = $sums['total_points_in'] ?? 0;
        $points_out_total = $sums['total_points_out'] ?? 0;
        return number_format($points_in_total - $points_out_total, 2);
    }
}

if (!function_exists('setUserSelectedSchool')){
    /**
     * @param School $school
     * @return void
     */
    function setUserSelectedSchool(School $school): void
    {
        $school->fresh();
...
if (!function_exists('studentProfileCompleted')){

    /**
     * @param User $user
     * @return bool
     */
    function studentProfileCompleted(User $user): bool
    {
        /*!empty($user->userBio->country_id) && !empty($user->userBio->city_id)
            && !empty($user->campus_id) && !empty($user->userBio->curriculum_id)
            && !empty($user->userBio->fee_range_id)*/
        return ($user->majors_count > 0 && $user->study_destinations_count > 0  && $user->preferred_universities_count > 0);
    }
}

if (!function_exists('getUserCountryCode')) {
    function getUserCountryCode(): array|string|null
    {
        $user_country_code = \Request::server('HTTP_CF_IPCOUNTRY');

        if (empty($user_country_code)) {
            $response = geoip(\Request::getClientIp());
            $user_country_code = $response->iso_code;
        }
...
if (!function_exists('setupLocale')) {
    function setupLocale($lang): void
    {

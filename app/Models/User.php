 ...
 * @property-read University|null $selected_university
 * @property-read University|null $university
 * @property-read \Illuminate\Database\Eloquent\Collection|User[] $universityLeads
 * @property-read int|null $university_leads_count
 * @property int|null $ministry_id
 * @method static \Illuminate\Database\Eloquent\Builder|User whereMinistryId($value)
 * @property-read mixed $currency_conversion_rate
 * @method static \Illuminate\Database\Eloquent\Builder|User whereStudents()
 ...
 * @method static \Illuminate\Database\Eloquent\Builder|User whereLeadSourceId($value)
 * @mixin \Eloquent
 */
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    protected $with = ['selected_university', 'userBio', 'roles','preferences.currency'];
    ...
    public function getCurrencyConversionRateAttribute(){
        if (!\Auth::check() || \Auth::id() != $this->id) return null;
        $currency = $this->preferences?->currency;
        if (empty($currency)) return null;
        return \Cache::remember(\AppConst::USER_CURRENCY_CONVERSION_RATE_CACHE."-".$this->id,Carbon::now()->addHours(6),
            function () use($currency){
                return Currency::convert()->from('USD')->to($currency->code)->round(2)->get();
            });
    }
    function scopeWhereStudents($query)
    {
    ...
    /**
     *  <b>For University Representatives Only</b>
     * @return BelongsTo
     */
     ...
     */
    public function universityLeads(): BelongsToMany
    {
        return $this->belongsToMany(User::class,UniversityLead::class,'add_by','student_id')
            ->withPivot('notes', 'add_by','created_at','status');
    }

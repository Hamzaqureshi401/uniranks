...
if (!function_exists('numberToWord')) {
    function numberToWord($num = ''): array|string
    {
...
if (!function_exists('calculateMatching')) {

    /**
     * @param User $student
     * @param array $uni_majors
     * @param $uni_cities
     * @param $uni_countries
     * @return int
     */
    function calculateMatching(User $student, array $uni_majors, $uni_cities, $uni_countries): int
    {
        if (is_array($uni_cities)) {
            $same_city = in_array($student->userBio?->city_id, $uni_cities);
        } else {
            $same_city = $student->userBio?->city_id == $uni_cities;
        }

        if (is_array($uni_countries)) {
            $same_country = in_array($student->userBio?->city_id, $uni_cities);
        } else {
            $same_country = $student->userBio?->country_id == $uni_countries;
        }

        $same_majors = !empty(array_intersect($uni_majors, $student->majors->pluck('id')->toArray()));
        $matching = 5;
        if ($same_country && $same_majors && !$same_city) {
            $matching = 7;
        } elseif ($same_city && $same_country && $same_majors) {
            $matching = 10;
        }
        return $matching;
    }

...
<x-print-layout :title="$title">
    <div class="row">
        <div class="col-12">
            @php
                /**
                * @var  \App\Models\Transactions\EventCreditTransaction[] $invoices
            ...
            <table style="width: 100%">
                ...
                <tr>
                    ...
                    <td style="text-align: right">
                        <h1 class="h1"><b>@lang('Event Credits List')</b></h1>
                    ...
                    <td colspan="2">
                        <table style="width: 100%; border: solid 1px #ccced0; border-collapse: collapse">
                            ...
                            <tr style="background: #1c345a !important; ">
                                ...
                                <td style="color: white; padding: .5rem .5rem;">@lang('Credit In')</td>
                                <td style="color: white; padding: .5rem .5rem;">@lang('Credit Out')</td>
                            ...
                            @forelse($invoices as $invoice)
                                ...
                                <tr>
                                 ...
                                 border-bottom: 1px solid #ccced0;">{{$invoice->created_at?->toDayDateTimeString()}}
                                    </td>
                                    @if($invoice->is_reverse)
                                        <td style="padding: .5rem .5rem;
                                 border-bottom: 1px solid #ccced0;"> @lang('Revers Credit From:') {{$invoice->event?->fair_name}}</td>
                                    @else
                                        <td>{{$invoice->event?->fair_name ?? $invoice->event_name}}</td>
                                    @endif
                                    <td style="padding: .5rem .5rem;
                                 border-bottom: 1px solid #ccced0;">{{$invoice->credit_in}}</td>
                                    <td style="padding: .5rem .5rem;
                                 border-bottom: 1px solid #ccced0;">{{$invoice->credit_out}}</td>
                                    <td style="padding: .5rem .5rem;
                                 border-bottom: 1px solid #ccced0;">{{$invoice->balance}}</td>
                </tr>
                @empty
                @endforelse
                </tbody>

<div>
    <x-general.university-facility-page-title sub-title="Transport  Facility"/>
    <div class="card mt-3">
        <div class="card-body">
            <div class="blue">@lang('University Transport Facility Information')</div>
            ...
            <div>
                @php
                    /**
                    * @var \App\Models\University\UniversityTransportType  [] $categories
                    * @var \App\Models\General\VehicleType [] $vehicle_types
                    * @var \App\Models\University\Facility\UniversityFacilityTransportation $facility_information
                    **/
                @endphp
                <div class="row mt-2">
                    <div class="col-md-6">
                        <div class="form-floating w-100">
                            <select wire:model.defer="facility_information.transport_type_id"
                                    class="form-select input-field">
                                <option value="">@lang('Transport Type')</option>
                                @foreach($categories as $category)
                                    <option value="{{$category->id}}">{{$category->name}}</option>
                                @endforeach
                            </select>
                            <label for="floatingSelectGrid">@lang('Transport Type')</label>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-floating w-100">
                            <select wire:model.defer="facility_information.vehicle_type_id"
                                    class="form-select input-field">
                            ...
                            <label for="floatingSelectGrid">@lang('Vehicle Type')</label>
                        </div>
                    </div>
                </div>
                        ...
                        <div @class(["mobile-marg-2 col-md-8","col-md-12"=>($i == 0)])>
                            <div class="form-floating w-100">
                                <input wire:model.defer="names.{{$i}}" class="form-control input-field"
                                       placeholder="@lang('Transport Facility Name')">
                                <label for="floatingInput">@lang('Transport Facility Name')</label>
                            </div>
                        </div>
                        @if($i > 0)
                            <div class="col-md-4">
                ...
                <div class=" text-place-end mt-4 mb-4">
                    <button class="m-0 button-no-bg" wire:click="addDetailsInOtherLanguage" type="button">
                        @lang('+ Add Transport Facility Information into different language')
                    </button>
                </div>
                <div class="row">
                    <div class="col-md-4 d-flex align-items-center">
                        <input wire:model.defer="facility_information.gender_based" type="checkbox" value="1">
                        <span class="ms-2 blue">@lang('Gender Separation Buses')</span>
                    </div>
                    <div class="col-md-8 d-flex align-items-center">
                        <input wire:model.defer="facility_information.wifi_available" type="checkbox" value="1" >
                        <span class="ms-2 blue">@lang('Free WIFI Network in university buses')</span>
                    </div>
                </div>
                <div class="row">
    ...
    @php
        /**
        * @var \App\Models\University\Facility\UniversityFacilityTransportation [] $dataCollection
        * @var \App\Models\University\Facility\UniversityFacilityTransportation $selected_item
        **/
    @endphp
    <div class="h4 blue mt-3" id="upload-images">@lang('Transport Facility Detail and Gallery')</div>
    <div class="row align-items-baseline">
        <div class="col-md-8">
            <select wire:model="item_id" wire:change="loadAlbumData" class="input-field form-control">
                <option value="">@lang('Select Transport Facility')</option>
    ...
    @if(!empty($selected_item))
        ...
        <livewire:university-facilities.components.add-transport-stops :languages="$languages"
                                                                       :transport="$selected_item"
                                                                       :wire:key="'stops-'.$selected_item->id"/>
        <div class="h4 blue mt-3">@lang('Upload Images')</div>
            ...
            <div class="card-body">
                <div class="w-100">
                    <div class="row">
                        ...
                        @foreach($selected_item?->stops as $stop)
                            <div class="row h6 mt-4">
                                <div class="col-md-4 blue">{{$stop->title}}</div>
                                <div class="col-md-6 mobile-marg-4 blue justify-content-between d-flex">
                                    {{$stop->times->implode('time',' | ')}}
                                </div>
                                <div class="col-md-2 mobile-marg-4 justify-content-mobile-start d-flex">
                                    <div class=""><a href="" wire:click.prevent="editStop('{{$stop->id}}')" class="light-blue">@lang('Edit')</a></div>
                                    <div class=""><a href="" wire:click.prevent="deleteStop('{{$stop->id}}')" class="red ms-3">@lang('Delete')</a></div>
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="h5 text-center blue d-flex justify-content-center">
                                ...
                                @lang('Select Transport facility to view or update Images')
                            </div>
                        </div>
                    </div>
                </div>

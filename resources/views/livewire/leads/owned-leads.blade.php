        ...
        <div class="card shadow-none bg-transparent2">
                <div id="select-div" x-show="showFilters" class="shadow-none" style="">
                    <div class="p-3 d-lg-flex align-items-end">
                        <div class="blue h5 mb-0 col-lg-3 col-md-4 col-12">@lang('Owned Leads')</div>
                            ...
                            <div
                                ...
                                <div class="dropdown btn-group gray">
                                    ...
                                    <div class="dropdown-menu  filter-dropdown gray shadow-none"
                                        ...
                                        <div class="dropdown dropend">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-last" aria-labelledby="dropdown-layouts">
                                ...
                                <div class="dropdown gray ms-2">
                                    <button class="m-0 px-1 gray my-dropdown-toggle d-flex align-items-end"
                                            type="button"
                                            data-bs-toggle="dropdown" aria-expanded="false">
                                        <span><i class="fa-solid fa-eye pe-1 gray"></i></span><span>@lang('Matching')</span>
                                    </button>
                                    <div class="dropdown-menu  filter-dropdown gray shadow-none"
                                         aria-labelledby="dropdownMenuButton">
                                        <!-- New Item -->
                    ...
                    <div class="p-3 d-lg-flex align-items-center">
                        <div class="col-9 d-flex align-items-center">
                            ...
                            <div class="width-14 mobile-btn">
                                <label class="button-xs button-no-bg3 w-95 p-1" for="select-all-leads">@lang('Select all') {{$students->total()}}</label>
                            </div>
                            @endif
                            <div class="width-14 mobile-btn">
                                <button wire:click="archiveLeads" class="button-xs button-no-bg3 w-95 p-1">@lang('Archive')</button>
                            </div>
                            <div class="width-14 mobile-btn">
                                <button wire:click="exportLeads" class="button-xs button-no-bg3 w-95 p-1"><span><i
                                            class="fa-solid fa-file-excel"></i></span> @lang('Export')
                                </button>
                            </div>
                            <div class="width-14 mobile-btn">
                                <button wire:click="openModalConfirmModal" class="button-xs button-no-bg3 w-95 p-1">@lang('Delete')</button>
                            </div>
{{--                            <div class="width-14 mobile-btn">--}}
{{--                                <button class="button-xs button-no-bg3 w-95 p-1">--}}
{{--                                    <strong>@lang('API')</strong>@lang('Push')--}}
{{--                                </button>--}}
{{--                            </div>--}}
                        </div>
                    </div>
                        ...
                        <tr>
                            <td scope="col" class="p-0 border-bottom2">
                                <div class="d-flex align-items-center p-2">
                                    <div>
                                        <input wire:model="selected_all" id="select-all-leads" value="1" wire:change="selectAll"
                            ...
                            <tr>
                                ...
                                <td class="">
                                    @if($student->studyDestinations->isEmpty())
                                        N/A
                                    @else
                                        {{$student->studyDestinations->first()->country_name}}
                                        <a href="#" @click.prevent="$wire.emit('showStudentDetails','{{$student->id}}')" class="light-blue">{{$student->studyDestinations->count()}}</a>
                                    @endempty
                                </td>
                                <td class="">
                                    @if($student->preferredUniversities->isEmpty())
                                        N/A
                                    @else
                                        {{$student->preferredUniversities->first()->university_name}}
                                        <a href="#" @click.prevent="$wire.emit('showStudentDetails','{{$student->id}}')" class="light-blue">{{$student->preferredUniversities->count()}}</a>
                                    @endempty
                                </td>
                                <td class="">
                                    @if($student->majors->isEmpty())
                                        N/A
                                    @else
                                        {{$student->majors->first()->title}}
                                        <a href="#" @click.prevent="$wire.emit('showStudentDetails','{{$student->id}}')" class="light-blue">{{$student->majors->count()}}</a>
                                    @endempty
                                </td>
                                {{--TODO REAL TIME Enroll Status--}}
            ...
            <div id="footer2">
                <div class="p-3 d-md-flex justify-content-end align-items-center">
                    <div x-show="!showFilters" id="footer-div-show" class="col-md-7">
                        <div class="d-flex">
                            <div
                                class="blue"><span x-text="selected_count"></span> @lang('applications have been selected')</div>
                            <div class="ps-4"><a class="gray text-decoration-none"><strong>API </strong>Push</a>
                            </div>
                            <div class="ps-2"><a href="javascript:void(0)" wire:click="exportLeads"
                                                 class="gray a-underline"><span><i class="fa-solid fa-file-excel"></i></span> @lang('Export')</a></div>
                        </div>
                    </div>
    ...
    <x-jet-modal wire:model="isModalOpen">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="gray paragraph-style2">
                        {{ __("Please note that, leads will be deleted permanently and can not be recovered") }}
                    </div>
                </div>
            </div>
        </x-slot>
        <div class="row mt-3 blue">
            <div class="col-12">
                @lang('Are you sure you want to delete') <b class="red">{{$selected_all ? 'all':count($selectedStudents) }}</b>  @lang('lead(s)?')
            </div>
        </div>
        <x-general.loading message="Processing..." wire:target="deleteLeads,closeModal" />
        <div class="row mt-3">
            <div class="col-3 offset-6">
                <button wire:click="deleteLeads" class="button-sm button-red w-100">
                    @lang('Yes Delete')
                </button>
            </div>
            <div class="col-3">
    ...
    <x-general.loading
        wire:target="exportLeads, openModalConfirmModal, archiveLeads, selectAll ,loadFilteredData, applyFilters, applyGroupBy, remove_filter, remove_group, previousPage, gotoPage, nextPage"
        message="Processing..."/>

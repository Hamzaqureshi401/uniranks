      ...
      <div class="card-body">
         ...
         <form>
            ...
            <div class="row" data-id="{{$i}}">
               <div class="col-md-6">
                  <div class="form-floating w-100">
                     <select wire:model.defer="translations.{{$i}}" class="form-select input-field" wire:change="setPrimaryAndSecondary({{$i}} , $event.target.value)" aria-label="">
               ...
               <div class="col-md-6" id="hide-{{$i}}">
                  
                  <div class="form-floating w-100">
                     <select wire:model.defer="name_type.{{$i}}" id='type-{{$i}}' class="form-select input-field" aria-label="">
            ...
            <div class="row mt-3">
               <div class="col-12">
                  <div class="form-floating w-100">
                     <input type="text" wire:model.defer="name.{{$i}}" class="form-control input-textarea" placeholder="@lang('University Name')" rows="7">
...
<script>
   $(document).ready(function() {
    ...
    document.addEventListener('livewire:load', function () {
    Livewire.on('checkResult', (result, index) => {
        console.log('Received result:', result); // true or false
        console.log('Received index:', index); // $i value
        if(result == false){
            $('#hide-'+index).removeClass('d-none');
        }else{
            $('#hide-'+index).addClass('d-none');
        }
        
        
        // Use the result and index as needed in your jQuery logic
    });
});
